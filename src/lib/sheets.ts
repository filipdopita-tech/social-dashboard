...
function getAuth() {
  ...
  if (process.env.GOOGLE_SA_BASE64) {
    credentials = JSON.parse(Buffer.from(process.env.GOOGLE_SA_BASE64, "base64").toString("utf-8"));
  } else if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
    credentials = JSON.parse(process.env.GOOGLE_SERVICE_ACCOUNT_JSON);
  } else {
    credentials = JSON.parse(fs.readFileSync("/root/social_poster/service_account.json", "utf-8"));
  }
